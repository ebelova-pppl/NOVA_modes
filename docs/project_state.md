...
- Active version-controlled training list:
    ...
    - 2263 labeled TAE-like modes: 592 `good`, 1671 `bad`
    - shots: `nstx_120113`, `nstx_135388`, `nstx_141711`, `nstxu_204202`,
      ...
      `nstxuG142301H47`, `nstxuG121123J38`
...
1.	RF (Random Forest)
    ...
    -	Checkpoint status: trained before the 2026-07-06 merge, on the previous
      2125-row / 10-shot active list; retraining on the current 2263-row /
      11-shot list is pending
    -	Current schema: previous RF features minus `omega`, plus `W_star_max`
    -	Previous expanded 10-shot OOF accuracy: 0.956
    -	Previous expanded 10-shot OOF CM: `[[1447, 29], [64, 585]]`
    -	Previous GOOD precision/recall/F1: 0.953 / 0.901 / 0.926
...
## Current tasks
- Retrain RF and raw CNN on the current 2263-row / 11-shot
  `training_labels/tae_like_train.csv` list after the refreshed
  `nstx_135388` replacement and `nstxuG121123J38` merge.
...
The active training list now has `2263` rows: `592` good and `1671` bad across
...
The RF and raw-CNN checkpoints still reflect the previous 2125-row / 10-shot
training list, so retraining is now pending before reporting new production
model metrics.
