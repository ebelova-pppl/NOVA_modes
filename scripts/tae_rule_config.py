...
from tae_rule_engine import (
    ...
    DISTRIBUTED_NOISE_RULESET_VERSION,
...
RULE_CONFIG_SCHEMA_VERSION = "tae-rule-run-config-v13"
PRODUCTION_RULE_CONFIG_NAME = "tae_rules_production_v13"
PRODUCTION_RULE_CONFIG_SHA256 = (
    "5d1319910b578d9b684a367d358d5a2304a7319218fe1571b462e9ce9d3b3919"
)
FROZEN_CONFIGURATION_SHA256 = {
    "tae_rules_production_v12": "6cec796ae20bac12f2f66bd18ac20a14d9e502aa64453c6f2b5ad10f7b54f925",
...
def load_rule_run_configuration(value: str | Path) -> RuleRunConfiguration:
    ...
    if schema_version not in {
        ...
        raise ValueError(
            ...
            f"expected {RULE_CONFIG_SCHEMA_VERSION!r}, v12, v11, v10, v9, v8, v7, v6, or v5"
        )
    ...
    expected_ruleset = {
        ...
        "tae-rule-run-config-v12": DISTRIBUTED_NOISE_RULESET_VERSION,
    ...
    _require_exact_keys(
        deduplication, {"rel_freq_tol"} | ({"rank_method"} if schema_version in {RULE_CONFIG_SCHEMA_VERSION, "tae-rule-run-config-v12", "tae-rule-run-config-v11"} else set()), context="deduplication"
    )
    rank_method = (_string(deduplication, "rank_method", context="deduplication")
                   if schema_version in {RULE_CONFIG_SCHEMA_VERSION, "tae-rule-run-config-v12", "tae-rule-run-config-v11"} else "rf_p_good")
    ...
    if schema_version in {RULE_CONFIG_SCHEMA_VERSION, "tae-rule-run-config-v12", "tae-rule-run-config-v11", "tae-rule-run-config-v10", "tae-rule-run-config-v9"}:
        gate_names.add("axis_energy_concentration")
    if schema_version in {RULE_CONFIG_SCHEMA_VERSION, "tae-rule-run-config-v12", "tae-rule-run-config-v11", "tae-rule-run-config-v10"}:
        gate_names.add("extended_continuum_noise")
    if schema_version in {RULE_CONFIG_SCHEMA_VERSION, "tae-rule-run-config-v12"}:
    ...
    _require_exact_keys(
        ...
        | (
            {"smooth_exception"}
            if schema_version in {RULE_CONFIG_SCHEMA_VERSION, "tae-rule-run-config-v12", "tae-rule-run-config-v11", "tae-rule-run-config-v10", "tae-rule-run-config-v9", "tae-rule-run-config-v8", "tae-rule-run-config-v7"}
            else set()
        ),
        context="gates.continuum_crossing_window",
    )
    ...
    _require_exact_keys(
        ...
        {"enabled", "r_edge_min", "width_max_grid"} | (
            {"secondary_peak_energy_min", "secondary_peak_body_r_max"} if schema_version == RULE_CONFIG_SCHEMA_VERSION else set()
        ),
    ...
    edge_secondary_min = (_float(edge, "secondary_peak_energy_min", context="gates.edge_artifact")
                          if schema_version == RULE_CONFIG_SCHEMA_VERSION else None)
    edge_body_r_max = (_float(edge, "secondary_peak_body_r_max", context="gates.edge_artifact")
                       if schema_version == RULE_CONFIG_SCHEMA_VERSION else DEFAULT_EDGE_SECONDARY_PEAK_BODY_R_MAX)
    EdgeArtifactConfig(
        secondary_peak_body_r_max=edge_body_r_max,
        secondary_peak_energy_min=edge_secondary_min,
        r_edge_min=edge_r,
    ...
    _require_exact_keys(
        interior,
        {
            ...
            "ext_df_gap_min",
            "ext_df_gap_max",
        } | ({"ext_df_gap_min_inclusive"} if schema_version in {RULE_CONFIG_SCHEMA_VERSION, "tae-rule-run-config-v12", "tae-rule-run-config-v11", "tae-rule-run-config-v10", "tae-rule-run-config-v9", "tae-rule-run-config-v8"} else set()) | ({"footprint_exception"} if schema_version == RULE_CONFIG_SCHEMA_VERSION else set()),
        context="gates.interior_unresolved_envelope",
    )
    ...
    interior_ext_df_min_inclusive = (
        ...
        if schema_version in {RULE_CONFIG_SCHEMA_VERSION, "tae-rule-run-config-v12", "tae-rule-run-config-v11", "tae-rule-run-config-v10", "tae-rule-run-config-v9", "tae-rule-run-config-v8"} else True
    )
    footprint_kwargs = dict(footprint_spikes_fraction_max=None,
                            footprint_local_hf_fraction_max=DEFAULT_INTERIOR_FOOTPRINT_LOCAL_HF_FRACTION_MAX,
                            footprint_window_dr=DEFAULT_INTERIOR_FOOTPRINT_WINDOW_DR)
    if schema_version == RULE_CONFIG_SCHEMA_VERSION:
        context = "gates.interior_unresolved_envelope.footprint_exception"
        footprint = _mapping(interior["footprint_exception"], context=context)
        _require_exact_keys(footprint, {"enabled", "spikes_fraction_max", "local_hf_fraction_max", "window_dr"}, context=context)
        footprint_kwargs = dict(footprint_spikes_fraction_max=_float(footprint, "spikes_fraction_max", context=context),
                                footprint_local_hf_fraction_max=_float(footprint, "local_hf_fraction_max", context=context),
                                footprint_window_dr=_float(footprint, "window_dr", context=context))
        InteriorUnresolvedEnvelopeConfig(**footprint_kwargs)
        if not _bool(footprint, "enabled", context=context):
            footprint_kwargs["footprint_spikes_fraction_max"] = None
    InteriorUnresolvedEnvelopeConfig(
        **footprint_kwargs,
    ...
    run_kwargs = {
        **{"distributed_noise_" + key: getattr(distributed_config, key)
        ...
        "edge_r_min": edge_r,
        "edge_secondary_peak_energy_min": edge_secondary_min,
        "edge_secondary_peak_body_r_max": edge_body_r_max,
        "edge_width_max_grid": edge_width if edge_enabled else None,
        **{"interior_envelope_" + k: v for k, v in footprint_kwargs.items()},
