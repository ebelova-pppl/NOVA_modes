...
def run_sort_fold(args: argparse.Namespace, shot: str, env: dict[str, str]) -> None:
    ...
    cmd = [
        *resolve_launch_prefix(args, args.sort_launch),
        ...
        str(paths["cnn_model"]),
        "--cnn_model_kind",
        "cnn_raw_continuum_branch" if args.cnn_continuum_branch else "cnn_raw",
        "--out_dir",
        str(paths["sort_dir"]),
        "--label_csv",
        str(paths["test_csv"]),
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    default_train_csv = Path(
        os.environ.get("NOVA_TRAIN_CSV", repo_root / "training_labels" / "tae_like_train.csv")
    )
    ...
    ap.add_argument("--cnn_batch_size", type=int, default=32)
    ...
    ap.add_argument(
        ...
        action="store_true",
        help=(
            "Train/evaluate the experimental raw CNN with a radius-aligned 1D "
            "branch containing W(r), du, dl, and a validity mask."
        ),
    )
    ap.add_argument(
        "--cnn_continuum_branch_zero_inputs",
    ...
    ap.add_argument(
        ...
        type=float,
        default=5.0,
        help="Clip value for experimental continuum-branch du/dl inputs (default: 5.0)",
    )
